...
const riskMeta: Record<SupplierRiskLevel, { label: string; tone: 'ok' | 'warn' | 'alert' }> = {
  low: { label: '저위험', tone: 'ok' },
  medium: { label: '중위험', tone: 'warn' },
  high: { label: '고위험', tone: 'alert' },
  critical: { label: '최고위험', tone: 'alert' },
};

const feocLabel: Record<string, string> = {
  eligible: '적격',
  ineligible: '부적격',
  under_review: '검토 중',
  unknown: '미파악',
...
// agreement_status → 자가평가 검토 상태
function toAssessmentStatus(rel: SupplierReliabilityResponse | null): AssessmentStatus {
  switch (rel?.agreementStatus) {
    case 'agreed': return 'approved';
    case 'rejected': return 'rework';
...
export default function SupplierReliabilityPage() {
    ...
    (async () => {
      ...
      try {
        ...
        const enriched = await Promise.all(
          visible.map(async (brief): Promise<ReliabilityRow> => {
            try {
              return { brief, reliability: await getSupplierReliability(brief.supplierId) };
            } catch {
        ...
        if (cancelled) return;
        setRows(enriched);
        setSelectedId(enriched[0]?.brief.supplierId ?? null);
      } catch (err) {
        if (!cancelled) {
          setError(err instanceof ApiError ? err.message : '신뢰성 데이터를 불러오지 못했습니다.');
        }
      } finally {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter(row => {
      const status = toAssessmentStatus(row.reliability);
      const matchesFilter = filter === 'all' || status === filter;
      const matchesQuery = !query || [row.brief.supplierId, row.brief.companyName]
        .filter(Boolean).join(' ').toLowerCase().includes(query);
      return matchesFilter && matchesQuery;
    });
  }, [rows, filter, search]);

  const selected = rows.find(r => r.brief.supplierId === selectedId) ?? rows[0] ?? null;
  const selRel = selected?.reliability ?? null;
  const selStatus = toAssessmentStatus(selRel);
  const selRiskLevel = selRel?.riskLevel ?? null;
  const completion = selRel?.completenessScore ?? 0;
          ...
          <aside className="flex min-h-0 flex-col border-b border-ink-700 bg-ink-800/60 xl:border-b-0 xl:border-r">
            ...
            <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
              {loading ? (
                ...
                <div className="rounded-xs border border-dashed border-red-200 bg-red-50 p-5 text-center text-xs text-red-700">
                  {error}
                </div>
                ...
                filteredRows.map(row => {
                  ...
                  const score = row.reliability?.completenessScore ?? 0;
                  return (
                    <button
                      key={row.brief.supplierId}
                      type="button"
                      onClick={() => setSelectedId(row.brief.supplierId)}
                      className={clsx(
                        'w-full rounded-xs border p-3 text-left transition-colors',
                        selected?.brief.supplierId === row.brief.supplierId
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="truncate text-xs font-bold text-ink-100">{row.brief.companyName}</div>
                          <div className="mt-0.5 truncate text-[11px] text-ink-500">{row.brief.supplierId}</div>
                        </div>
                        <Badge tone={meta.tone}>{meta.label}</Badge>
                      </div>
                      <div className="mt-3 flex items-center justify-between gap-2 text-[11px]">
                        <span className="num-mono text-ink-500">완성도</span>
                        <span className="num-mono font-bold text-ink-300">{score}%</span>
                      </div>
                    </button>
                  );
                })
          ...
          <main className="min-w-0 bg-white p-6 xl:p-7">
            {!selected ? (
              <div className="flex h-full items-center justify-center text-xs text-ink-500">
                {loading ? '불러오는 중…' : '선택된 협력사가 없습니다.'}
              </div>
            ) : (
              <>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-ink-100">{selected.brief.companyName}</h2>
                    <p className="mt-1 text-xs text-ink-500">{selected.brief.supplierId}</p>
                  </div>
                  <Badge tone={statusMeta[selStatus].tone} size="md">
                    {statusMeta[selStatus].label}
                  </Badge>
                </div>

                <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
                  <MetricCard label="종합 리스크 점수" value={selRel?.overallRiskScore ?? '—'} unit="/ 100점" tone="info" />
                  <MetricCard label="데이터 입력 완성도" value={completion} unit="%" tone="ok" />
                  <MetricCard
                    label="공급망 리스크 평가"
                    value={selRiskLevel ? riskMeta[selRiskLevel].label : '미파악'}
                    tone={selRiskLevel ? riskMeta[selRiskLevel].tone : 'info'}
                  />
                </div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-5 2xl:grid-cols-2">
                  <Card title="기업 세부 프로필" subtitle="평가 판단에 필요한 기본 기준 정보">
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <Mini label="협력사 계층" value="—" />
                      <Mini label="공급망 위험도" value={selRiskLevel ? riskMeta[selRiskLevel].label : '미파악'} />
                      <Mini label="FEOC 자격 요건" value={selRel?.feocStatus ? feocLabel[selRel.feocStatus] : '미파악'} />
                      <Mini label="SLA 기한" value={selRel?.slaDueDate?.slice(0, 10) ?? '—'} />
                    </div>
                    <div className="mt-4 rounded-xs border border-ink-700 bg-ink-800/50 p-3">
                      <div className="text-xs font-bold text-ink-100">검토 이력</div>
                      <div className="mt-1 text-[11px] text-ink-500">
                        실사 {selRel?.totalAudits ?? 0}건
                        {selRel?.lastAuditDate && ` · 최근 ${selRel.lastAuditDate.slice(0, 10)}`}
                        {selRel?.lastAuditResult && ` · 결과 ${selRel.lastAuditResult}`}
                      </div>
                      <div className="mt-1 text-[11px] text-ink-400">담당자 정보: 연동 대기 중</div>
                    </div>
                  </Card>
                  ...
                  <Card title="컴플라이언스 리스크 시그널" subtitle="검토 우선순위를 결정하는 핵심 신호">
                    <div className="space-y-3">
                      <Signal
                        tone={selRel?.isHighRiskFlag ? 'warn' : 'ok'}
                        title={selRel?.isHighRiskFlag ? '고위험 플래그 설정됨' : '현재 고위험 플래그 없음'}
                        desc={selRel?.isHighRiskFlag
                          ? '고위험 플래그가 설정되어 추가 검토가 필요합니다.'
                          : '등록된 고위험 플래그가 없습니다.'}
                      />
                      <Signal
                        tone={selRel?.lastAuditResult === 'fail' ? 'warn' : 'ok'}
                        title="최근 실사 결과"
                        desc={selRel?.lastAuditResult
                          ? `최근 실사 결과: ${selRel.lastAuditResult} (총 ${selRel?.totalAudits ?? 0}건)`
                          : '등록된 실사 결과가 없습니다.'}
                      />
                    </div>
                  </Card>
                ...
                <div className="mt-5 grid grid-cols-1 gap-5 2xl:grid-cols-[1.25fr_0.75fr]">
                  <Card title="증빙 제출 현황" subtitle="필수·선택 증빙 자료 검토 상태">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      {evidenceItems.map((item, index) => {
                        const done = completion - index * 10 >= 50;
                        return (
                  ...
                  <Card title="검토 결과와 조치" subtitle="결과는 협력사 목록과 DPP 판단에 반영됩니다">
                    <div className="space-y-2">
                      {selRel?.isHighRiskFlag ? (
                        <div className="flex items-start gap-2 rounded-xs border border-red-200 bg-red-50 p-3">
                          <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-red-700" />
                          <div className="text-xs leading-5 text-red-900">고위험 플래그가 설정되어 있습니다. 실사·증빙을 우선 검토하세요.</div>
                        </div>
                      ) : (
