...
## 4. `percentage` — §10.2a로 이관 + 백엔드 develop 구현 완료 ✅

**결정(2026-06-26)**: BOM의 `percentage`는 BOM에서 채우지 않는다. 이 값의 진짜 의미는 BOM 구성비중이 아니라
**협력사·공장별 공급 비율**이며, §10.2a 엔드포인트로 노출된다.
...
- **백엔드 develop 반영(2026-06-26 재검토)**: §10.2a `GET /products/{id}/supply-chain-map`가 **구현 완료**(main.py 등록). 응답에 `supplyChainRatios[].ratioPercent`(엣지 내 공장 분할) + `cumulativeContribution`(루트→공장 경로 곱) + 계층별 100% `validation` 포함.
- **상대값/절대값 논쟁 종결**: develop `repository.get_supply_chain_contributions` 주석에 *"ratio_percentage는 직속 부모 대비 상대값(PM 확정) → 경로 곱이 말단 기여도"* 명시. SVG 예시(c=20×30%=6%, d=14%, e=40%, f=40%) 그대로 산출됨. → **상대값 + 누적 곱 확정**.
- 프론트 소비: `buildTraceRows`의 `supply_ratio` = §10.2a `ratioPercent`. `bom_percentage`는 BOM(0) 유지. 화면 비율 표기는 §10.2a 값으로 간다.
